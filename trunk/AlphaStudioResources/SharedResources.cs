...
using AlphaStudioResources.Properties;
using ClientCommonWpf;
using Color = System.Windows.Media.Color;

namespace AlphaStudioResources
{
	/// <summary>
	...
	public static class SharedResources
	{
		...
		private static readonly Dictionary<string , ImageSource> m_images = new Dictionary<string, ImageSource>();

		public static Dictionary<int, Color> ChangeColors = new Dictionary<int, Color>();

		static SharedResources()
		{
			foreach (var info in typeof(Resources).GetProperties(BindingFlags.Static | BindingFlags.NonPublic))
			{
				if (info.PropertyType == typeof(Bitmap))
				{
					m_images.Add("Img" + info.Name, ((Bitmap)info.GetValue(null, new object[0])).Source());
					m_images.Add("Img" + info.Name + "Disabled", ((Bitmap)info.GetValue(null, new object[0])).SourceDisabled());
				}
			}
		}

		static ResourceDictionary GetResourceDictionary(string _dictionaryName)
		{
			...
			var result = Application.LoadComponent(new Uri(assemblyName + ";component/" + _dictionaryName + ".xaml", UriKind.Relative)) as ResourceDictionary;


			for (var index = 0; index < result.MergedDictionaries.Count; index++)
			{
